 ...
 *
 * @param paramsRef 响应式参数对象。当内部属性变化时（如 keywords），TanStack Query 会自动重置并重新请求
 * @param initialPageNum 初始页码，默认为 1
 */
export function useArticleList(paramsRef: Ref<IUseArticleListParams>, initialPageNum = 1) {
  // 使用 useInfiniteQuery 处理无限滚动逻辑
  return useInfiniteQuery({
    // 1. Query Key (查询键)
    // 类似于 Vue 的 watch 依赖。
    // 当 paramsRef.value 中的任何属性变化时（比如 keywords 变了），
    ...
    // 无感切换：如果您切走再切回来（在一定时间内），组件会直接渲染缓存的数据，而不是像以前那样必须显示 Skeleton 骨架屏等待 Loading。
    // Stale-While-Revalidate：它采用“先显示旧数据，后台静默更新”的策略，体验比单纯的“清空 -> Loading -> 显示”要流畅得多。
    // 这个 queryKey 就是缓存的"钥匙"
    queryKey: ['articles', paramsRef],

    // 2. Query Function (查询函数)
    // 接收 pageParam（当前页码），调用实际的 Service 层接口
    queryFn: async ({ pageParam = initialPageNum }) => {
      // 清理参数中的 undefined/null/空字符串，避免 URL 过长或参数混乱
      const cleanParams = Object.fromEntries(Object.entries(paramsRef.value).filter(([_, v]) => v !== undefined && v !== null && v !== ''));
      ...
      const res = await getList({
        ...
        pageSize: paramsRef.value.pageSize || 10, // 默认 pageSize
      });
    ...
    // 4. Get Next Page Param (计算下一页页码)
    // 替代了 Store 里的 `hasMore` 判断和 `pageNum + 1` 逻辑
