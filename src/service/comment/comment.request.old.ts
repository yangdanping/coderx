...
export function getComment(data) {
  ...
  console.log('getComment params', { articleId, userId });
  return myRequest.get<IResData>({
    url: `${urlHead}?articleId=${articleId}&userId=${userId}`,
  });
}

export function updateCommentOld(commentInfo) {
...
export function addCommentOld(commentInfo) {
...
export function addReplyOld(replyInfo) {
...
export function likeCommentOld(commentId) {
  return myRequest.post<IResData>({
    url: `${urlHead}/${commentId}/like`,
  });
}

export function getCommentByIdOld(commentId) {
  return myRequest.get<IResData>({
    url: `${urlHead}/${commentId}`,
  });
}
