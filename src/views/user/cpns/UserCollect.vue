<template>
  <div class="user-collect">
    <UserCollectArticles v-if="activeCollect" :key="`${ownerId}:${activeCollect.id}`" :collect="activeCollect" :can-manage="isMe" @back="clearResultAndBack" />
    <template v-else>
...
import UserCollectArticles from './UserCollectArticles.vue';
...
const ownerId = computed(() => Number(route.params.userId));

// 判断是否为当前登录用户（用于控制收藏夹操作权限）
const isMe = computed(() => isCurrentUser(ownerId.value));

const sex = computed(() => (profile.value.sex === '男' ? '他' : '她'));
const activeCollectId = ref<number | null>(null);
const activeCollect = computed(() => collects.value.find((item) => item.id === activeCollectId.value) ?? null);
...
const goCollectDetial = (item: ICollect) => {
  cancelCreate();
  activeCollectId.value = item.id;
};

const clearResultAndBack = async () => {
  cancelCreate();
  activeCollectId.value = null;
  if (!Number.isSafeInteger(ownerId.value) || ownerId.value <= 0) return;
  try {
    await userStore.getCollectAction(ownerId.value);
  } catch {
    Msg.showFail('收藏夹刷新失败，请稍后重试');
  }
};
